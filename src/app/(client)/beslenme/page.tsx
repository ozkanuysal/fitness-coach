...
export const metadata = { title: "Beslenme - FitKoc" };

export default async function BeslenmePage() {
  const session = await auth();
  await dbConnect();

  const programs = await MealProgram.find({ danisanId: session?.user?.id })
    ...
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground">
          Beslenme Programlarim
        </h1>
        <p className="text-muted mt-1">Sana ozel hazirlanmis beslenme programlarin</p>
      </div>

      {programs.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {programs.map((p: any) => (
            <Link key={p._id.toString()} href={`/beslenme/${p._id}`}>
                ...
                <div className="flex items-center justify-between text-sm text-muted">
                  ...
                  {p.makroHedefleri?.gunlukKalori && (
                    <span className="text-primary">
                      {p.makroHedefleri.gunlukKalori} kcal/gun
                    </span>
                  )}
                </div>
              </Card>
        ...
        <EmptyState
          title="Henuz beslenme programin yok"
